...
describe('deleteFile', () => {
  ...
  it('should delete file and sync pyodide.runPythonAsync FS', async () => {
    ...
    expect(pyodide.runPythonAsync).toHaveBeenCalledWith(`import db_worker; db_worker.delete_file('/${fileName}')`);
  ...
  it('should handle errors during file deletion', async () => {
    ...
    await expect(deleteFile(fileName)).rejects.toThrow('Deletion error');
    expect(pyodide.runPythonAsync).toHaveBeenCalledWith(`import db_worker; db_worker.delete_file('/${fileName}')`);
