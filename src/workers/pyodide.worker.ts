...
try {
  self.onmessage = async (event: MessageEvent<WorkerInMessage>) => {
    console.log("[pyodide-worker] Message received:", event.data);
    ...
    switch (msg.type) {
      ...
      case "IMPORT_FILES": {
        console.log("[pyodide-worker] IMPORT_FILES received", { fileName: msg.fileName, fileContent: msg.fileContent });
        try {
          if (!pyodide) await initPyodide();
          const filePath = `${DEFAULT_WORK_DIR}/${msg.fileName}`;
          if (msg.fileContent instanceof Uint8Array) {
            pyodide.FS.writeFile(filePath, msg.fileContent);
          } else {
            pyodide.FS.writeFile(filePath, msg.fileContent, { encoding: "utf8" });
          }
          await syncVfsToIDB();
          // Return file metadata for all files in DEFAULT_WORK_DIR
          const files = pyodide.FS.readdir(DEFAULT_WORK_DIR).filter((f: string) => f !== "." && f !== "..");
          const fileMeta = files.map((fileName: string) => {
            const stat = pyodide.FS.stat(`${DEFAULT_WORK_DIR}/${fileName}`);
            return {
              fileName,
              filePath: `${DEFAULT_WORK_DIR}/${fileName}`,
              fileSize: stat.size,
              createdAt: stat.ctime * 1000
            };
          });
          self.postMessage({ type: "IMPORT_FILES_RESPONSE", payload: fileMeta });
        } catch (err) {
          console.error("[pyodide-worker] Error during IMPORT_FILES:", err);
          self.postMessage({ type: "IMPORT_FILES_RESPONSE", payload: [] });
        }
        break;
      }

      case "DELETE_FILE": {
        console.log("[pyodide-worker] DELETE_FILE received", { filePath: msg.filePath });
        try {
          if (!pyodide) await initPyodide();
          pyodide.FS.unlink(`${DEFAULT_WORK_DIR}/${msg.filePath}`);
          await syncVfsToIDB();
          self.postMessage({ type: "DELETE_FILE_RESPONSE", payload: true });
        } catch (err) {
          console.error("[pyodide-worker] Error during DELETE_FILE:", err);
          self.postMessage({ type: "DELETE_FILE_RESPONSE", payload: false });
        }
        break;
      }

      case "SAVE_FILE": {
        console.log("[pyodide-worker] SAVE_FILE received", { fileName: msg.fileName });
        try {
          if (!pyodide) await initPyodide();
          const filePath = `${DEFAULT_WORK_DIR}/${msg.fileName}`;
          pyodide.FS.writeFile(filePath, msg.fileContent, { encoding: "utf8" });
          await syncVfsToIDB();
          self.postMessage({ type: "SAVE_FILE_RESPONSE", payload: true });
        } catch (err) {
          console.error("[pyodide-worker] Error during SAVE_FILE:", err);
          self.postMessage({ type: "SAVE_FILE_RESPONSE", payload: false });
        }
        break;
      }

      case "LOAD_FILES": {
        console.log("[pyodide-worker] LOAD_FILES received");
        try {
          if (!pyodide) await initPyodide();
          const files = pyodide.FS.readdir(DEFAULT_WORK_DIR).filter((f: string) => f !== "." && f !== "..");
          const fileMeta = files.map((fileName: string) => {
          ...
          self.postMessage({ type: "LOAD_FILES_RESPONSE", payload: fileMeta });
        } catch (err) {
          console.error("[pyodide-worker] Error during LOAD_FILES:", err);
          self.postMessage({ type: "LOAD_FILES_RESPONSE", payload: [] });
        }
        break;
      }

      case "READ_FILE": {
        console.log("[pyodide-worker] READ_FILE received", { filePath: msg.filePath });
        try {
          if (!pyodide) await initPyodide();
          const filePath = `${DEFAULT_WORK_DIR}/${msg.filePath}`;
          const content = pyodide.FS.readFile(filePath, { encoding: "utf8" });
          self.postMessage({ type: "READ_FILE_RESPONSE", payload: { filePath: msg.filePath, content } });
        } catch (err) {
          console.error("[pyodide-worker] Error during READ_FILE:", err);
          self.postMessage({ type: "READ_FILE_RESPONSE", payload: { filePath: msg.filePath, content: null } });
        }
        break;
      }

      case "CLEAR_FILES": {
        console.log("[pyodide-worker] CLEAR_FILES received");
        try {
          if (!pyodide) await initPyodide();
          const files = pyodide.FS.readdir(DEFAULT_WORK_DIR).filter((f: string) => f !== "." && f !== "..");
          for (const file of files) {
            pyodide.FS.unlink(`${DEFAULT_WORK_DIR}/${file}`);
          }
          await syncVfsToIDB();
          self.postMessage({ type: "CLEAR_FILES_RESPONSE", payload: true });
        } catch (err) {
          console.error("[pyodide-worker] Error during CLEAR_FILES:", err);
          self.postMessage({ type: "CLEAR_FILES_RESPONSE", payload: false });
        }
        break;
      }
