...
def train_and_select_iteration(args: Args) -> None:
	...
	# Define file paths
	PATHS: dict[str, str] = {
		# Unlabeled file paths (src)
		'TRAIN_SRC': f'{sub_datadir}/train.100.src',
		'TEST_SRC': f'{args['datadir']}/{args['lang']}/test.100.full.src',
		'SELECT_SRC': f'{sub_datadir}/select.100.src',
		# Labeled file paths (tgt)
		'TRAIN_TGT': f'{sub_datadir}/train.tgt',
		'TEST_TGT': f'{args['datadir']}/{args['lang']}/test.100.full.tgt',
		# Prediction file paths
		'TEST_PRED': f'{sub_datadir}/test.full.pred',
		'SELECT_PRED': f'{sub_datadir}/select.pred',
...
def setup_datadirs(args: Args) -> str:
	...
	sub_datadir: str = f'{args['datadir']}/{args['lang']}/{args['seed']}/{args['method']}/iter{args['curr_al_iter']}'
...
def split_increment_residual(confidence_data: list[ConfidenceData], increment_size: int) -> tuple[list[ConfidenceData], list[ConfidenceData]]:
	"""
	...
	"""
	increment_data: list[ConfidenceData] = confidence_data[:increment_size]
	residual_data: list[ConfidenceData] = confidence_data[increment_size:]
...
def output_crf(crf: sklearn_crfsuite.CRF, sub_datadir: str, args: Args, data: DataDict, X_test: DatasetFeatures) -> tuple[DatasetLabels, DatasetLabels]:
	"""
	...
	if data['select']['words']:
		X_select, _, _ = get_features(data['select']['words'], data['select']['bmes'], args['delta'])
		...
		increment_data, residual_data = split_increment_residual(conf_sorted_data, args['increment_size'])
...
def build_and_output_crf(sub_datadir: str, args: Args, data: DataDict) -> tuple[DatasetLabels, DatasetLabels]:
	"""
	...
	:rtype: tuple[DatasetLabels, DatasetLabels]
	"""
	X_train, Y_train, _ = get_features(data['train']['words'], data['train']['bmes'], args['delta'])
	X_test, _, _ = get_features(data['test']['words'], data['test']['bmes'], args['delta'])
